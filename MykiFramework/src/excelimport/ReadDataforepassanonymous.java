...
public class ReadDataforepassanonymous {

	public ReadDataforepassanonymous() throws Exception {
		super();
	...
	int row_count;
	DataFieldValuesforepassanonymous datasheet=null;
	protected List<DataFieldValuesforepassanonymous>datasheetList=new ArrayList<DataFieldValuesforepassanonymous>();
	Row row=null;
	XSSFCell cell=null;
	public List<DataFieldValuesforepassanonymous>readDataFromExcel(String filepath, String sheetname) throws Exception{
		...
		for (int i=1;i<=row_count;i++){
			row=sheet.getRow(i);
			datasheet= new DataFieldValuesforepassanonymous();
			...
			String Duration = df.formatCellValue(cell);
			datasheet.setDuration(Duration);
			
			cell = (XSSFCell) row.getCell(2);
			String zonemin = df.formatCellValue(cell);
			datasheet.setZoneLow(zonemin);
			
			cell = (XSSFCell) row.getCell(3);
			String zonemax = df.formatCellValue(cell);
			datasheet.setZoneHigh(zonemax);
